...
def ensure_sam3_model():
    # Naudojame SAM 3.1 Multiplex modelį
    model_path = "sam3.1_multiplex.pt"
    
    if not os.path.exists(model_path):
        print("SAM 3.1 modelis nerastas! Prašome įkelti 'sam3.1_multiplex.pt' į projekto aplanką.", flush=True)
        sys.exit(1)
...
class SAMAutomaticAnalyzer:
    def __init__(self, image_path):
        ...
        # 2. Inicializuojame SAM 3.1 Multiplex modelį
        print("Inicializuojamas Segment Anything 3.1 Multiplex modelis GPU atmintyje...", flush=True)
        try:
            from sam3 import build_sam3_image_model
            from sam3.model.sam1_task_predictor import SAM3InteractiveImagePredictor
        except ImportError:
            print("KLAIDA: Nėra įdiegtos 'sam3' bibliotekos.", flush=True)
            print("Prašome įdiegti: pip install sam3", flush=True)
            sys.exit(1)

        model_path = ensure_sam3_model()
        device = "cuda" if torch.cuda.is_available() else "cpu"
        if device == "cpu":
            print("ĮSPĖJIMAS: CUDA nerasta! SAM 3.1 veikia labai lėtai be GPU.", flush=True)
        else:
        ...
        # Priskiria backbone interaktyviam prediktoriui (SAM3 reikalavimas)
        if hasattr(self.sam_model, 'inst_interactive_predictor') and self.sam_model.inst_interactive_predictor is not None:
            self.sam_model.inst_interactive_predictor.model.backbone = self.sam_model.backbone
            self.predictor = self.sam_model.inst_interactive_predictor
        else:
            self.predictor = SAM3InteractiveImagePredictor(self.sam_model)
    ...
    def generate_masks(self):
        """Automatiškai segmentuoja nuotrauką naudojant SAM 3.1 su grynuoju algoritmu."""
        import time
        from sam3.perflib.nms import nms_masks as sam3_nms
        from tqdm import tqdm
        
        print(f"\n--- VYKDOMAS AUTOMATINIS APTIKIMAS (SAM 3.1, Bandymas #{self.attempt_count}) ---", flush=True)
        print(f"Laikas: {time.strftime('%H:%M:%S')}", flush=True)
        
        # Tankis pagal bandymą: 32 → 48 → 64 taškų tinklelis
        pps = 32 + (self.attempt_count - 1) * 16
        print(f"Tinklelio tankis: {pps} taškų pusėje ({pps*pps} vnt.).", flush=True)
        
        h, w = self.image_rgb.shape[:2]
        x = np.linspace(0, w, pps + 2)[1:-1]
        y = np.linspace(0, h, pps + 2)[1:-1]
        xv, yv = np.meshgrid(x, y)
        points = np.stack([xv.flatten(), yv.flatten()], axis=1)
        
        # Nustatome paveikslėlį prediktoriui (vienas kartas)
        self.predictor.set_image(self.image_rgb)
        
        # Vykdome nuspėjimą BATCHED rėžimu per predict_batch (daug greičiau)
        all_masks_np = []
        all_scores_np = []
        batch_size = 64
        
        for i in tqdm(range(0, len(points), batch_size), desc="SAM 3.1 segmentuoja"):
            batch_pts = points[i:i+batch_size]                              # [B, 2]
            batch_coords = batch_pts[:, np.newaxis, :]                      # [B, 1, 2]
            batch_labels = np.ones((len(batch_pts), 1), dtype=np.int32)    # [B, 1]
            
            with torch.no_grad():
                # predict_batch grąžina: list of (masks, scores, logits) – po vieną batch elementui
                results = self.predictor.predict_batch(
                    point_coords_batch=list(batch_coords),
                    point_labels_batch=list(batch_labels),
        ...
        if not all_masks_np:
            print("Klaida: Nerasta jokių objektų.", flush=True)
            self.masks = []
            return

        print(f"Rasta pradinių kaukių: {len(all_masks_np)}. Vykdomas SAM3 NMS filtravimas...", flush=True)

        scores_t = torch.tensor(all_scores_np, dtype=torch.float32)
        
        # Sumažiname kaukes NMS tikslais (CPU)
        masks_small_list = []
        for m in all_masks_np:
            m_t = torch.from_numpy(m).unsqueeze(0).unsqueeze(0).float()
            m_small = torch.nn.functional.interpolate(
                m_t, size=(256, 256), mode='bilinear', align_corners=False
            ).squeeze() > 0.5
            masks_small_list.append(m_small)
        masks_small_t = torch.stack(masks_small_list)

        # Naudojame oficialią SAM3 NMS iš sam3.perflib
        keep = sam3_nms(
            pred_probs=scores_t,
            pred_masks=masks_small_t,
            prob_threshold=0.5,
            iou_threshold=0.7
        )
        ...
        self.masks = []
        for idx in keep:
            i = int(idx)  # garantuojame Python int (ne tensorius)
            m_np = all_masks_np[i]
            self.masks.append({
                'segmentation': m_np,
                'area': int(np.sum(m_np)),
                'predicted_iou': float(scores_t[i].item())
            })
        
        print(f"Detekcija baigta! Po filtravimo liko {len(self.masks)} unikalių objektų.", flush=True)
    ...
    def calculate_statistics(self, save_files=True):
        ...
        print(f"\n=== MIKROSTRUKTŪROS STATISTIKA (SAM 3.1 - Atnaujinta {now}) ===", flush=True)
        if len(stats_list) > 0:
